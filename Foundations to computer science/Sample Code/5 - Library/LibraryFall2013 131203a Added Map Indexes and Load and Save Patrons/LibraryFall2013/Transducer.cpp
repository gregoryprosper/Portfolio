...
string Transducer::transduce(string command)
{
	try
	{
		...
		else if((token[0]=="LOADPATRONS")||(token[0]=="LP"))
		{
			if(token.size()!=2)
			{
				throw "ERROR: Wrong Number of Parameters.";
			...
			{
				result=library.loadPatrons(token[1]);
			}
		}
		else if((token[0]=="SAVEPATRONS")||(token[0]=="SP"))
		{
			...
			else
			{
				...
				for(int i=1;i<token.size();i++)
				{
					result+=library.savePatrons(token[i])+"\n";
				}
			}
		}
